...
def multiprocessing(corrected, val):
    indices = str([0, 1, 2, 3, 4])
    for each in range(len(corrected)):
        current_stack[val].append(corrected[each])

        logger.info("length of current_stack: " + str(len(current_stack[val])))

        if len(current_stack[val]) == 7:
            holo_overlays(current_stack[val], indices)

            logger.info("Number of frames in frames_output: " + str(len(frames_output)))
            del current_stack[val][:]
    del corrected[:]
...
def holo_overlays(rgb, val):
    ...
    for i in range(len(rgb)):
        ...
        all_op[val].append(overlayed_img)
...
if __name__ == '__main__':
    ...
    current_stack = [stack1, stack2, stack3, stack4, stack5]
    op1, op2, op3, op4, op5 = [], [], [], [], []
    all_op = [op1, op2, op3, op4, op5]
    val = str([0, 1, 2, 3, 4])

    frame_job(video)
    print("corrected images array length: " + str(len((corrected))))

    divide = int(len(corrected)/5)
    start, end = 0, divide
    for i in range(5):
        all_list[i].append(corrected[start:end])
        start += divide
        end += divide

    p = Pool(5)
    p.map(multiprocessing, all_list, val)
